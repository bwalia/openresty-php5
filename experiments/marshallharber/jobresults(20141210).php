...
if($cat_id=$db->get_var("select ID from categories where Code='".$cat_code."' and SiteID=".SITE_ID." and type='job' and Active=1")){
	$query="select d.* from documentcategories dc join documents d on dc.DocumentID=d.ID where dc.CategoryID=".$cat_id." and dc.SiteID=".SITE_ID." and d.SiteID=".SITE_ID." and d.type='job' and d.PublishCode=1";
	$limit = 10;
	...
	<h1><a class="jobboard_heading_link" href="content.php?code=<?php echo $doc_cat->Code; ?>.html" title="<?php echo $doc_cat->Document; ?>"><?php echo $doc_cat->Document; ?></a>&nbsp;&nbsp;&nbsp;&nbsp;

	<a class="jobboard_add_remove_link" href="javascript:_updateMyJobsList('addRemove<?php echo $count; ?>', <?php echo $doc_cat->ID; ?>, 'user_selection_documents_jobapplist');" id="addRemove<?php echo $count; ?>" title="Add this job to your list"></a>
	...
	<p class="more"><?php if(strlen(strip_tags($doc_cat->Body))>200) { echo substr(strip_tags($doc_cat->Body),0,200).'&hellip;'; } else { echo substr(strip_tags($doc_cat->Body),0,200); } ?><a class="jobboard_heading_link" href="content.php?code=<?php echo $doc_cat->Code; ?>.html" title="<?php echo $doc_cat->Document; ?>">more</a></p>
	</div>
	<?php 
			} 
		}
