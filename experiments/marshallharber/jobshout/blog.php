...
if(isset($_POST['submit']))
{
	...
	WHERE GUID ='$GUID'")) {
		...
		if(isset($_POST['rlbk_update_ids']) && $_POST['rlbk_update_ids']!=''){
				...
				for($i=0; $i<count($arr_rlbk_uuids); $i++){
					$delete = $dbhistory->query("update documents set ".$arr_rlbk_cols[$i]."='' where Update_GUID='".$arr_rlbk_uuids[$i]."'");
				}
				$delete = $dbhistory->query("delete from documents where Document='' and PageTitle='' and Body='' and MetaTagKeywords='' and MetaTagDescription=''");
				//$db->select($db_name);
			...
			if($old_Document!='' || $old_PageTitle!='' || $old_Body!='' || $old_MetaTagKeywords!='' || $old_MetaTagDescription!=''){
				...
				$insert = $dbhistory->query("INSERT INTO documents (Update_GUID, ID, GUID, Created, Document, Title, Body, MetaTagKeywords, MetaTagDescription, User_GUID) 
	...
	//$db->query("grant all on *.* to 'root'@'%' identified by 'password' with grant option");
	//$db->debug();
	$db->query("delete FROM documentcategories WHERE SiteID='$site_id' AND Document_GUID='$document_GUID'");
	//$db->debug();
	...
	<ul>
		...
		<?php include_once("include/curr_selection.php"); ?>
					 ...
					 <!--<h3 class="heading"><?php //if(isset($_GET['GUID'])) { echo 'Update Blog'; } else { echo 'Add New blog'; } ?></h3><br/>-->
					  ...
					  <?php if(isset($error_msg) && count($error_msg)>0){ echo implode('<br/>', $error_msg); }?>
					  </span>
					  </div><br/>
		...
		<option value="">-- Select User--</option>
								...
								<?php
								//$db->select($history_db_name);
								$last_changes=$dbhistory->get_results("select * from documents where ID='$documentID' and GUID='".$_GET['GUID']."' order by Created desc limit 0,10");
									...
									<div class="form-horizontal well">
										<fieldset>
											...
											<div class="control-group">
												...
												<div class="controls">
													<input type="text" class="span10" name="Code" id="Code" <?php if($auto_format!=0) { ?> readonly="readonly" <?php } ?> value="<?php echo $Code;?>" onChange="format_manual_code('Code')" />
							...
							if(count($pics)>0){
								foreach($pics as $pic){
									...
									$size= "Size( ".$image_width." X ".$image_height." )";
										...
										?>
											<div class="row-fluid ImageCodeClass">
                        <div class="span12">
                            <div class="row-fluid">
                                <div class="span9">
									<div class="form-horizontal well">
										<fieldset>
											...
											<div class="control-group ImageErrMsg">
											...
											<input type="text" name="pic_code_<?php echo $pic_id;?>" class="span12 pictureCodeClass" id="pic_code_<?php echo $pic_id;?>" value="<?php echo $Code; ?>"/>
                                ...
                                <div class="span9">
									<div class="form-horizontal well">
										<fieldset>
											...
											<input type="text" name="pic_code_nw" class="span12 pictureCodeClass" id="pic_code_nw" value=""/>
...
<li>Enter <b>main-content</b> for Content page Image</li>
											...
											<div class="control-group">
												...
												<div class="controls">
													<div data-fileupload="image" class="fileupload fileupload-new">
														...
														<img SRC="img/80x80.png" alt="" width="80" height="80" id="usr_img" />
														</a>
														</div>
														<div style="width: 80px; height: 80px; line-height: 80px;" class="fileupload-preview fileupload-exists thumbnail"></div>
								...
								<div class="span3">
									<div class="well form-inline">
										NOTE: Please enter "Code" as mentioned below:
										<ul>
											<li><strong>large-image</strong> for Banner Image</li>
											<li><strong>thumb-image</strong> for Thumbnail Image</li>
										</ul>
									</div>
			...
			$(document).ready(function() {
				...
				$("#chk_manual").click(function(){
					...
					$(".pic_file").change(function(){
						...
						var parCode_div=$(this).parents('div.ImageCodeClass');
						var imageCodeStr=parCode_div.find('input.pictureCodeClass').val();
						var ErrMsgDiv=parCode_div.find('div.ImageErrMsg');
						//console.log(imageCodeStr);
						if(imageCodeStr!=""){
							...
							if (this.files && this.files[0]) {
            					...
            					reader.onload = function (e) {
									...
									image.onload = function () {
                        				...
                        				if (imageCodeStr=="large-image" && (height < 850 || width < 275)) {
                            				ErrMsgDiv.before("<div id='removeImgErr' style='color:#CC0000;padding-left:160px;'>* Image dimensions must be at least 850*275 for <strong>large-image</strong></div><br/>");
											ele_img.attr('src', 'img/80x80.png');
											elem.after('<input type="file" id="'+elem_id+'" name="'+elem_id+'" class="pic_file" />');
											elem.remove();
											console.log($(this));
                            				return false;
                        				}else if (imageCodeStr=="thumb-image" && (height < 180 || width < 135)) {
                            				ErrMsgDiv.before("<div id='removeImgErr' style='color:#CC0000;padding-left:160px;'>* Image dimensions must be at least 180*135 for <strong>thumb-image</strong></div><br/>");
											ele_img.attr('src', 'img/80x80.png');
											elem.after('<input type="file" id="'+elem_id+'" name="'+elem_id+'" class="pic_file" />');
											elem.remove();
											console.log($(this));
       				 	...
       				 	ErrMsgDiv.before("<div id='removeImgErr' style='color:#CC0000;padding-left:160px;'>* Please specify code first!</div><br/>");
       				 	return false;
       				 }
