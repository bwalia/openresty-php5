	...
	WHERE GUID ='$GUID'")) {
		...
		else
		{
			if($ShortUrl=='' || !$db->get_row("select * from documents where short_url='$ShortUrl'"))
			{
				...
				$insert = $db->query("INSERT INTO documents (ID,SiteID, Created, Modified,Document,Code,short_url,Reference,Body,MetaTagKeywords,MetaTagDescription,UserID,Title,PageTitle,Type,Published_timestamp,GUID,Sync_LastTime,AutoFormatTitle,PostedTimestamp,Template, Status, Site_GUID, User_GUID, FFAlpha80_1,FFAlpha80_2,FFAlpha80_3,FFAlpha80_4,AutoFormatMetaData) 
										VALUES(NULL,'".$site_id. "','$time','$time', '$new_Document','$Code','$ShortUrl','$reference','$new_Body','$new_MetaTagKeywords','$new_MetaTagDescription','$UserID','$new_PageTitle', '$new_WindowTitle','$Type','$Published_timestamp','$GUID',1,$auto_format,'$Published_timestamp','$template', '$Active', '$site_guid', '$user_guid', '$FFAlpha80_1', '$FFAlpha80_2', '$FFAlpha80_3', '$FFAlpha80_4','$AutoFormatMetaData')");
...
?>
 ...
 <style>
			...
			*, *:before, *:after { margin: 0; padding: 0; box-sizing: border-box; }
			body { color: #B9B5C7; font: 14px 'Open Sans', sans-serif; }
                            ...
                            <div class="row-fluid">
                                <div class="span9">
									...
									<input type="hidden" value="<?php echo $documentID; ?>" name="doc_id" class="textbox">
											...
											<a class="popup-trigger" >Style Guide</a>
											
											<div class="popup">
		 <p> Madrid is a city in Europe and the capital and largest city of Spain. The population of the city is almost 3.2 million and that of the Madrid metropolitan area, around 6.3 million. <br><br>It is the third-largest city in the European Union, after London and Berlin, and its metropolitan area is the third-largest in the European Union after Paris and London. The city spans a total of 604.3 km2 (233.3 sq mi).</p>
		...
		<option value="">-- Select User--</option>
								...
								<span class="help-block">&nbsp;</span>
										...
										<?php if(isset($GUID) && $GUID!='' && isset($websiteAddress) && $websiteAddress!=''){	?>
											<a href="<?php echo $websiteAddress.'file:///content.php?code='.$Code;?>" target="_blank" style="background:none repeat scroll 0 0 #48a6d2; color: #fff; text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.2);" class="btn">Preview changes</a>
										<?php } ?>									
									</div>
