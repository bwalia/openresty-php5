...
if(isset($_POST['submit']))
{
	...
	$error_msg[]="This short url already exists.";
		...
		else
		{
			...
			{
				if($ShortUrl==''){				
				$insert = $db->query("INSERT INTO documents (ID,SiteID, Created, Modified,Document,Code,Body,MetaTagKeywords,MetaTagDescription,UserID,Title,PageTitle,Type,Published_timestamp,GUID,Sync_LastTime,AutoFormatTitle,PostedTimestamp,Template, Status,FFAlpha80_1,FFAlpha80_2,FFAlpha80_3,FFAlpha80_4) 
										VALUES(NULL,'".$site_id. "','$time','$time', '$new_Document','$Code','$new_Body','$new_MetaTagKeywords','$new_MetaTagDescription','$UserID','$new_PageTitle', '$new_WindowTitle','$Type','$Published_timestamp','$GUID',1,$auto_format,'$Published_timestamp','$template', '$Active', '$FFAlpha80_1', '$FFAlpha80_2', '$FFAlpha80_3', '$FFAlpha80_4')");
				}
				else{
	...
	if($_POST['obj_guid_nw']!='' && $_POST['obj_code_nw']!=''){
		$chk_obj=$db->get_var("select count(*) as num from objects where Code='".$_POST['obj_code_nw']."' and (`DocumentID`=$documentId || `Document_GUID`='".$document_GUID."') and SiteID='".$site_id."'");
			if($chk_obj>0){
				...
				$insert_obj=$db->query("insert into objects(`SiteID`, `Created`, `Modified`, `Code`, `TextObject`, `DocumentID`, `UserID`, `Title`, `Order`, `Type`, `GUID`, `Document_GUID`, `Status`, `AutoFormat`) values('".$site_id."', '$time','$time', '".addslashes($_POST['obj_code_nw'])."', '".addslashes($_POST['ObjectText_nw'])."', $documentId, $UserID, '".addslashes($_POST['ObjectHeading_nw'])."', '".$obj_order_by."', '".$_POST['Type_nw']."', '".$_POST['obj_guid_nw']."', '".$document_GUID."', '1', $obj_auto_format)");	
			}
	}
