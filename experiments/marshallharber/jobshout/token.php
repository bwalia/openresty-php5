...
{ 
	...
	$new_value=addslashes(str_replace("http://cdn.jobshout.com","",$_POST["TokenText"]));
	...
	WHERE GUID ='$guid'")) {
		...
		if(isset($_POST['rlbk_update_ids']) && $_POST['rlbk_update_ids']!=''){
			...
			for($i=0; $i<count($arr_rlbk_uuids); $i++){
				$delete = $dbhistory->query("update tokens set ".$arr_rlbk_cols[$i]."='' where Update_GUID='".$arr_rlbk_uuids[$i]."'");
			}
			
			$delete = $dbhistory->query("delete from tokens where Description='' and TokenText=''");
			//$db->debug();
			$db->select($db_name);
		}
		...
		if($old_description!='' || $old_value!='' || $old_code!=''){
			...
			$dbhistory->query("INSERT INTO tokens (Update_GUID, GUID, SystemID, ID, Created, SiteID, Description, TokenText, Code, User_GUID)
	...
	<ul>
		...
		<?php include_once("include/curr_selection.php"); ?>
                    ...
                    <div class="row-fluid">
						...
						<?php
							//$db->select($history_db_name);
							$last_changes=$dbhistory->get_results("select * from tokens where ID='$token_id' and GUID='".$_GET['GUID']."' order by Created desc limit 0,10");
