	...
	if(isset($_GET['GUID'])) {
		...
		if($chk=$db->get_row("select * from documents where Code='$Code' and SiteID='".$site_id."' and GUID<>'$guid'")){
			...
			if(isset($_POST['rlbk_update_ids']) && $_POST['rlbk_update_ids']!=''){
				...
				$delete = $dbhistory->query("delete from documents where Document='' and PageTitle='' and Body='' and MetaTagKeywords='' and MetaTagDescription='' and FFAlpha80_1=''");
				$db->select($db_name);		
			}
				...
				if($old_Document!='' || $old_PageTitle!='' || $old_Body!='' || $old_MetaTagKeywords!='' || $old_MetaTagDescription!='' || $old_location!=''){
					...
					$insert = $dbhistory->query("INSERT INTO documents (Update_GUID, ID, GUID, Created, Document, Title, Body, MetaTagKeywords, MetaTagDescription, FFAlpha80_1, User_GUID) 
									...
									<div class="form-horizontal well">
										...
										<div class="control-group">
												...
												<div class="controls">
													...
													<input type="text" class="span10" name="Document" id="Document" onKeyUp="generate_job_code('chk_manual');" onBlur="generate_job_code('chk_manual');  generate_metatags();" value="<?php echo $Document;?>" />
				...
				<option <?php if($job_type == 'Part time') { echo "selected"; } ?> value='Part time' >Part time</option>
											...
											<div class="control-group">
												<label class="control-label">Location<span class="f_req">*</span></label>
													...
													<input type="text" class="span10" name="FFAlpha80_1" id="FFAlpha80_1" onKeyUp="generate_job_code('chk_manual')" onBlur="generate_job_code('chk_manual'); generate_metatags();" value="<?php echo $location;?>" style="width:200px;" />
													<span class="help-block">&nbsp;</span></div>
...
<select name="category5" id="category5" class="cats" onChange="generate_metatags()"><option value="">-- Select Category --</option>
<?php echo $select_cats; ?>
</select>

<select name="category6" id="category6" class="cats" onChange="generate_metatags()"><option value="">-- Select Category --</option>
<?php echo $select_cats; ?>
</select>	
			...
			function generate_metatags(){
				...
				if(status!=true){
					...
					//var	jsonRow= "return_metatags_data.php?document_name="+document_name+"&body="+body+"&category1="+category1+"&category2="+category2+"&category3="+category3+"&category4="+category4+"&category5="+category5+"&category6="+category6+"&site_id="+site+"&location="+location;
					
					$.ajax({                
						type: "POST",
						url: "return_metatags_data.php",
						data: {site_id : site, document_name: document_name, body :body, category1 : category1, category2: category2 , category3:category3 , category4:category4 ,category5:category5 , category6:category6 , location:location },
						cache: false,
						dataType: "json",  
