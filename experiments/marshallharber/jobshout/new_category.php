...
{ 
	...
	$cat_group_id=$db->get_var("SELECT GUID FROM `categorygroups` WHERE ID=".$category_gropuID."");
	$user_name=$db->get_var("SELECT Name FROM `users` WHERE ID=".$user_id."");

	if($Active==1){ $status="Active"; } else { $status="Inactive"; }
	
	$fts=$Name.' '.$cat_group_name.' '.$reg_type.' '.$user_name.' '.$status.' '.$cat_group_id;
	/*FTS*/
	
	
	
	 $db->query("INSERT INTO categories (GUID,Created,Modified,SiteID,Name,Code,CategoryGroupID,Active,UserID,Type,FTS,Sync_Modified) VALUES ('$GUID','$time','$time','".SITE_ID."','$Name','$CODE','$category_gropuID','$Active','$user_id','$reg_type','$fts','$Sync_Modified')");
	// $db->debug();
