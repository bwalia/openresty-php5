...
//$encrypted_mypassword=md5($password);
	...
	if($db->num_rows>0){
		...
		$site_id='';
		if($site_code!=''){
			if($site_details=$db->get_row("select * from sites where code='$site_code'")){
		...
		elseif($LoginData->Site_GUID!='' || $LoginData->SiteID!=''){
			echo "select * from sites where GUID = '".$LoginData->Site_GUID."' or ID = '".$LoginData->SiteID."'";
			die();
			...
			echo "select id from sites where GUID in (select uuid_site from wi_user_sites where uuid_user='$user_guid')";
			die();
