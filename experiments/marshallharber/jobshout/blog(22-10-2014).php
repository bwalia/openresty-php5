...
//	$new_Body=addslashes(str_replace("http://cdn.jobshout.com","",$_POST["Body"]));
	...
	if($chk=$db->get_row("select * from documents where Code='$Code' and SiteID='".$site_id."'")){
		...
		$insert = $db->query("INSERT INTO documents (ID,SiteID,Document,Code,Body,MetaTagKeywords,MetaTagDescription,UserID,Title,PageTitle,Type,Published_timestamp,GUID,Sync_LastTime,AutoFormatTitle,PostedTimestamp,Created,Modified, Status,Template, Site_GUID, User_GUID, FFAlpha80_1,FFAlpha80_2,FFAlpha80_3,FFAlpha80_4) 
									VALUES(NULL,'$site_id','$new_Document','$Code','$new_Body','$new_MetaTagKeywords','$new_MetaTagDescription','$UserID','$new_PageTitle','$WindowTitle','$Type','$Published_timestamp','$GUID',1,$auto_format,'$Published_timestamp','$time','$time','$Active','$template', '$site_guid', '$user_guid', '$FFAlpha80_1', '$FFAlpha80_2', '$FFAlpha80_3', '$FFAlpha80_4')");
	}
...
if(isset($_GET['GUID'])) {
$GUID= $_GET['GUID'];
$document = $db->get_row("SELECT ID,SiteID,Document,Code,Document,Header,Body,MetaTagKeywords,MetaTagDescription,UserID,Title,PageTitle,Type,SiteName,GUID,Site_GUID,Reference,Sync_LastTime, AutoFormatTitle,
