...
require_once("lib.inc.php");
	...
	$set_session= set_session_values('web_enquiries',$sSearch,$iDisplayStart,$iDisplayLength);
	 ...
	 */
	$aColumns = array('web_enquiries.GUID','web_enquiries.Name','web_enquiries.Email','web_enquiries.Telephone','web_enquiries.enquiry_type','web_enquiries.zStatus','web_enquiries.Modified','sites.Name as SiteName','web_enquiries.ID');
	 
	$aColumns1 = array('GUID','Name','Email','Telephone','enquiry_type','zStatus','Modified','SiteName','ID');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "web_enquiries.GUID";
	
	/* DB table to use */
	$sTable = "web_enquiries left outer join sites on web_enquiries.SiteID=sites.ID";
	...
	if(isset($_SESSION['site_id']) && $_SESSION['site_id']!='')
	 {
		$sWhere = "WHERE web_enquiries.SiteID in ('".$_SESSION['site_id']."')";
	}
	else
	{
		$sWhere = "WHERE 1";
	...
	if(isset($_GET['en_type']) && $_GET['en_type']!=''){
		$sWhere .=" and web_enquiries.enquiry_type = '".$_GET['en_type']."' ";
	}
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns1) ; $i++ )
		{
			...
			{
				if($aRow[ $aColumns1[$i] ]=='ACTIVE'){ $row[] = 'Unread'; }
				else{ $row[] = 'Read'; }
			}
...
if($user_access_level>1) {
		$row[] = '<a href="enquiry.php?GUID='.$curr_id.'" title="Edit this enquiry" ><i class="splashy-pencil"></i><a>';
		$row[]= '<a href="delete-enquiry.php?GUID='.$curr_id.'" title="Delete this enquiry" onClick="return confirm(\'Are you sure to delete\');"><i class="splashy-error_x"></i><a>';
		}
