...
require_once("lib.inc.php");
	 ...
	 */
	$aColumns = array('documents.ID','sites.Name', 'documents.Document','documents.PageTitle', 'documents.Modified',"CONCAT( wi_users.firstname, ' ', wi_users.lastname ) as UserName", 'documents.PostedTimestamp','documents.Status','documents.GUID', 'sites.WebsiteAddress', 'documents.Code','documents.Body');
	
	$aColumns1 = array('WebsiteAddress', 'Code', 'ID', 'Name', 'Document', 'PageTitle', 'Modified','UserName', 'PostedTimestamp', 'Status', 'GUID');
	...
	$set_session= set_session_values('blogs',$sSearch,$iDisplayStart,$iDisplayLength);
	 ...
	 */
	 ...
	 {
		$sWhere = "WHERE documents.SiteID in ('".$_SESSION['site_id']."') and documents.Type='blog'";
	}
	else
	{
		$sWhere = "WHERE documents.Type='blog'";
	}
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns1) ; $i++ )
		{
			...
			elseif ($aColumns1[$i] == "PostedTimestamp" || $aColumns1[$i] == "Modified") {
			...
			/*elseif ($aColumns1[$i] == "UserID" ) {
				...
				$sql_name=mysql_query("select name from users where id=".$aRow[ $aColumns1[$i] ]."");
		...
		if($user_access_level>1) {	
			$row[] = '<a href="blog.php?GUID='.$curr_id.'" title="Edit this blog" ><i class="splashy-pencil"></i><a>';
			$row[]= '<a href="delete-blog.php?GUID='.$curr_id.'" title="Delete this blog" onClick="return confirm(\'Are you sure to delete\');"><i class="splashy-remove"></i><a>';
		}
