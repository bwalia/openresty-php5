...
require_once("lib.inc.php");
	...
	$set_session= set_session_values('wi_users',$sSearch,$iDisplayStart,$iDisplayLength);
	 ...
	 */
	 ...
	 {
		$sWhere = "WHERE (SiteID in ('".$_SESSION['site_id']."') OR uuid in ( select uuid_user from wi_user_sites where uuid_site in ( select GUID from sites where ID in ('".$_SESSION['site_id']."') )))";
	}
	else
	{
		$sWhere = "WHERE 1";
