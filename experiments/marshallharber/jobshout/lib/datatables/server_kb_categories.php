...
require_once("lib.inc.php");
	...
	$set_session= set_session_values('kb_categories',$sSearch,$iDisplayStart,$iDisplayLength);
	 ...
	 */
	$aColumns = array('kb_categories.ID', 'sites.Name as SiteName', 'kb_categories.name', 'kb_categories.Modified', 'kb_categories.status', 'kb_categories.uuid');
	 
	$aColumns1 = array('ID', 'SiteName', 'name', 'Modified', 'status', 'uuid');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "kb_categories.uuid";
	
	/* DB table to use */
	$sTable = "kb_categories left outer join sites on kb_categories.SiteID=sites.ID";
	 ...
	 */
	 ...
	 {
		$sWhere = "WHERE kb_categories.SiteID in ('".$_SESSION['site_id']."')";
	}
	else
	{
		$sWhere = "WHERE 1";
...
if($user_access_level>1) {
				$row[] = '<a href="kb_category.php?GUID='.$aRow[ $aColumns1[$i] ].'" title="Edit this category" ><i class="splashy-pencil"></i><a>';
				$row[]= '<a href="delete-kbcategory.php?GUID='.$aRow[ $aColumns1[$i] ].'" title="Delete this category" onClick="return confirm(\'Are you sure to delete\');"><i class="splashy-remove"></i><a>';
				}
