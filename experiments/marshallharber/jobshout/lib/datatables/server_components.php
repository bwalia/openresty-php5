...
require_once("lib.inc.php");
	...
	$set_session= set_session_values('components',$sSearch,$iDisplayStart,$iDisplayLength);
	 ...
	 */
	$aColumns = array('templates.ID', 'sites.Name as SiteName', 'templates.Name', 'templates.Code', 'templates.Description', 'templates.Modified','templates.Status', 'templates.GUID', 'templates.TemplateBLOB');
	 
	$aColumns1 = array('ID', 'SiteName', 'Name', 'Code', 'Description', 'Modified','Status', 'GUID');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "templates.GUID";
	
	/* DB table to use */
	$sTable = "templates left outer join sites on templates.SiteID=sites.ID";
	 ...
	 */
	 ...
	 {
		$sWhere = "WHERE templates.SiteID in ('".$_SESSION['site_id']."') and templates.Type=1";
	}
	else
	{
		$sWhere = "WHERE templates.Type=1";
	}
...
if($user_access_level>1) {
		$row[] = '<a href="component.php?GUID='.$curr_id.'" title="Edit this component" ><i class="splashy-pencil"></i><a>';
		$row[]= '<a href="delete-component.php?GUID='.$curr_id.'" title="Delete this component" onClick="return confirm(\'Are you sure to delete\');"><i class="splashy-remove"></i><a>';
		}
		else
		{
