...
require_once("lib.inc.php");
	...
	$set_session= set_session_values('links',$sSearch,$iDisplayStart,$iDisplayLength);
	 ...
	 */
	 $aColumns = array( 'links.ID', 'sites.Name as SiteName', 'links.Label', 'links.Link', 'links.short_url', 'links.Modified', 'links.Active', 'links.GUID');
	 
	$aColumns1 = array( 'ID', 'SiteName', 'Label', 'Link', 'short_url', 'Modified', 'Active', 'GUID');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "links.GUID";
	
	/* DB table to use */
	$sTable = "links left outer join sites on links.SiteID=sites.ID";
	 ...
	 */
	 ...
	 {
		$sWhere = "WHERE links.SiteID in ('".$_SESSION['site_id']."')";
	}
	else
	{
		$sWhere = "WHERE 1";
...
if($user_access_level>1) {
				$row[] = '<a href="link.php?GUID='.$aRow[ $aColumns1[$i] ].'" title="Edit this link" ><i class="splashy-pencil"></i><a>';
				$row[]= '<a href="delete-link.php?GUID='.$aRow[ $aColumns1[$i] ].'" title="Delete this link" onClick="return confirm(\'Are you sure to delete\');"><i class="splashy-remove"></i><a>';
				}
