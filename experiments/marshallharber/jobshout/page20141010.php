	...
	$error_msg[]="This short url already exists.";
		...
		else
		{
			if($ShortUrl=='' || !$db->get_row("select * from documents where short_url='$ShortUrl'"))
			{
				...
				$insert = $db->query("INSERT INTO documents (ID,SiteID, Created, Modified,Document,Code,short_url,Body,MetaTagKeywords,MetaTagDescription,UserID,Title,PageTitle,Type,Published_timestamp,GUID,Sync_LastTime,AutoFormatTitle,PostedTimestamp,Template, Status, Site_GUID, User_GUID, FFAlpha80_1,FFAlpha80_2,FFAlpha80_3,FFAlpha80_4) 
										VALUES(NULL,'".$site_id. "','$time','$time', '$new_Document','$Code','$ShortUrl','$new_Body','$new_MetaTagKeywords','$new_MetaTagDescription','$UserID','$new_PageTitle', '$new_WindowTitle','$Type','$Published_timestamp','$GUID',1,$auto_format,'$Published_timestamp','$template', '$Active', '$site_guid', '$user_guid', '$FFAlpha80_1', '$FFAlpha80_2', '$FFAlpha80_3', '$FFAlpha80_4')");
	...
	if($_POST['obj_code_nw']!=''){
		...
		$chk_obj=$db->get_var("select count(*) as num from objects where Code='".addslashes($_POST['obj_code_nw'])."' and (`DocumentID`=$documentId || `Document_GUID`='".$document_GUID."')");
			if($chk_obj>0){
				...
				$insert_obj=$db->query("insert into objects(`SiteID`, `Created`, `Modified`, `Code`, `TextObject`, `DocumentID`, `UserID`, `Title`, `Order`, `Type`, `GUID`, `Document_GUID`, `Status`, `AutoFormat`,  `Site_GUID`, `User_GUID`) values('".$site_id."', '$time','$time', '".addslashes($_POST['obj_code_nw'])."', '".addslashes($_POST['ObjectText_nw'])."', $documentId, '$UserID', '".addslashes($_POST['ObjectHeading_nw'])."', '".$obj_order_by."', '".$_POST['Type_nw']."', '".$obj_guid_nw."', '".$document_GUID."', '1', '$obj_auto_format', '$site_guid', '$user_guid')");
	...
	if(isset($_GET['GUID']) && isset($_POST['pic_id']) && isset($_POST['pic_guid'])){
			if(is_array($_POST['pic_id']) && is_array($_POST['pic_guid'])){
				...
				if(isset($_POST["pic_status_".$curr_id])) {
					...
					$update_obj=$db->query("update pictures set Modified='$time', Code='".$_POST['pic_code_'.$curr_id]."', Status='".$pic_status."', Order_By='".$_POST['pic_order_by_'.$curr_id]."', SiteID='".$site_id."', Site_GUID= '$site_guid' where `GUID`='".$_POST['pic_guid'][$i]."'");
	...
	if($_POST['pic_code_nw']!=''){
		/*$chk_obj=$db->get_var("select count(*) as num from pictures where Code='".$_POST['pic_code_nw']."' and (`DocumentID`=$documentId || `Document_GUID`='".$document_GUID."') and SiteID='".$site_id."'");
			...
			$pic_guid_nw= UniqueGuid('pictures', 'GUID');
				...
				$insert_obj=$db->query("insert into pictures(GUID,Created,Modified,SiteID,Name,Code,Status,Type,DocumentID,Picture,Order_By, Site_GUID) values('".$pic_guid_nw."', '$time','$time', '".$site_id."', '', '".$_POST['pic_code_nw']."', '".$pic_status."', '', $documentId, '', '".$_POST['pic_order_by_nw']."', '$site_guid')");
						...
						if(site_id!=''){
							$.ajax({
								...
								success: function(response){						
									...
									tiny_options['moxiemanager_rootpath']= "/var/www/vhosts/"+response+"/";
									tiny_options['moxiemanager_path']= "/var/www/vhosts/"+response+"/";
