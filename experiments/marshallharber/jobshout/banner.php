...
{ 
	...
	//echo "update categorygroups set SiteID='".$site_id."', Modified='$time', Name='$Name',Code='$CODE',Active='$Active',UserID='$user_id',Sync_Modified='$Sync_Modified',Type='$Type', Auto_Format=$Auto_Format where GUID='$guid'";
	 ...
	 $update = $db->query("update pictures set SiteID='".$site_id."', Modified='$time',Code='$CODE',Status='$Active',Order_By=$order_by, Site_GUID= '$site_guid' where GUID='$GUID'");
	 ...
	 //$db->debug();
		...
		$GUID=UniqueGuid('pictures', 'GUID');
		 ...
		 $insert = $db->query("INSERT INTO pictures (GUID,Created,Modified,SiteID,Name,Code,Status,Type,Picture,Order_By,Site_GUID) VALUES ('$GUID','$time','$time','".$site_id."','','$CODE','$Active','','',$order_by,'$site_guid')");
	...
	<ul>
		...
		<?php include_once("include/curr_selection.php"); ?>
					...
					<!--<h3 class="heading"><?php //if(isset($_GET['GUID'])) { echo "Update"; } else { echo "Add"; } ?> Category Group</h3>-->
							...
							<?php if(isset($_SESSION['up_message']) && $_SESSION['up_message']!=''){ echo $_SESSION['up_message']; $_SESSION['up_message']=''; }?></span></div><br/>
                    <div class="row-fluid">
