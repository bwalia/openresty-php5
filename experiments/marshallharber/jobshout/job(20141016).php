...
if(isset($_POST['submit']))
{
	...
	if($chk=$db->get_row("select * from documents where Code='$Code' and SiteID='".$site_id."'")){
		...
		$insert = $db->query("INSERT INTO documents (ID, SiteID, Created, Modified, Document, Code, Body, Reference, FFAlpha80_3, FFReal01, FFReal02, FFAlpha80_7, FFAlpha80_1, MetaTagKeywords, MetaTagDescription, UserID, Title, PageTitle, Type, Published_timestamp, GUID, Sync_LastTime, AutoFormatTitle, PostedTimestamp, post_code, latitude, longitude,status, PublishCode, Template, Site_GUID, User_GUID) 
									VALUES(NULL, '".$site_id."','$time','$time', '$new_Document', '$Code', '$new_Body', '$reference', '$job_type', '$sal_from', '$sal_to', '$sal_dur', '$new_location', '$new_MetaTagKeywords', '$new_MetaTagDescription', '$UserID', '$new_PageTitle', '$new_WindowTitle', '$Type', '$Published_timestamp', '$GUID', 1, '$auto_format', '$Published_timestamp','$post_code','$latitude','$longitude','$Active','$job_status', '$template', '$site_guid', '$user_guid')");
...
if(isset($_GET['GUID'])) {
$guid= $_GET['GUID'];
	$document = $db->get_row("SELECT ID, SiteID, Document, Code, Body, Reference, FFAlpha80_3, FFReal01, FFReal02, FFAlpha80_7, FFAlpha80_1, Title,MetaTagKeywords, MetaTagDescription, UserID, PageTitle, Type, Published_timestamp, GUID, Sync_LastTime,AutoFormatTitle, PostedTimestamp, post_code, latitude, longitude, status, PublishCode, Template FROM documents where documents.GUID ='$guid'");
	...
	<ul>
		...
		<option value="">-- Select Job Type--</option>
				...
				<option <?php if($sal_dur == 'per hour') { echo "selected"; } ?> value='per hour' >Per Hour</option>
											...
											<?php 
												if($session_site_guid = $db->get_var("select GUID from sites where ID = '".$_SESSION['site_id']."'")){
