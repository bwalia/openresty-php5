<?php
	 ...
	 */
	$aColumns = array('Code', 'Document', 'Published_timestamp','GUID');
	...
	/*local*/
	$gaSql['user']       = "jobshout";
	$gaSql['password']   = "!ChBi2K1ngS";
	$gaSql['db']         = "jobshout_live";
	$gaSql['server']     = "hh2.jobshout.co.uk";
	
	//server
	/*$gaSql['user']       = "root";
	$gaSql['password']   = "SanJose^D";
	$gaSql['db']         = "jobshout_live";
	$gaSql['server']     = "hh4.jobshout.co.uk";*/
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			elseif ($aColumns[$i] == "Published_timestamp") {
				$row[ $aColumns[$i] ] = Date('m-d-Y', $aRow[ $aColumns[$i] ] );
