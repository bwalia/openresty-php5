...
function get_cats($id,$guid){
	...
	global $sWhere;
	$cat_html='';
	if($categories= $db->get_results("select * from categories where $sWhere and (TopLevelID='".$id."' or TopLevel_GUID='".$guid."') order by Name asc")){		
...
function get_docs($id,$guid){
	...
	global $docWhere;
	$doc_html='';
	if($documents= $db->get_var("select count(documents.GUID) from documents join documentcategories on (documents.ID=documentcategories.DocumentID or documents.GUID=documentcategories.Document_GUID) where $docWhere and (documentcategories.CategoryID='".$id."' or documentcategories.Category_GUID='".$guid."') and documents.Type='page'")){
                   ...
                   <?php if(isset($_SESSION['ins_message']) && $_SESSION['ins_message']!=''){ echo $_SESSION['ins_message']; unset($_SESSION['ins_message']); }?>
					...
					if($cat_groups= $db->get_results("select * from categorygroups where $sWhere order by Name asc")){						
						foreach($cat_groups as $cat_group){							
							if($categories= $db->get_results("select * from categories where $sWhere and (CategoryGroupID='".$cat_group->ID."' or CategoryGroup_GUID='".$cat_group->GUID."') and (TopLevelID='0' and TopLevel_GUID='') order by Name asc")){
